...
def validate_integrity_stamp(message: memoryview, stamp: bytes, crypo_key: bytes) -> bool:
    ...
    if provided_stamp_size != expected_stamp_size:
        raise RuntimeError(
            f'Integrity stamp has wrong size. Expected {expected_stamp_size}, received {provided_stamp_size}.')
...
def generate_hash(message: memoryview, crypo_key: bytes, salt: bytes = None) -> tuple[bytes, bytes]:
    from .parameters import INTEGRITY_STAMP_HASH_SIZE, \
        INTEGRITY_STAMP_SALT_SIZE
    ...
    hasher = blake2b(
        message, digest_size=INTEGRITY_STAMP_HASH_SIZE, key=crypo_key, salt=salt)
